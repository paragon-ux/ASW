...
DEFAULT_PROFILE = ROOT / "fixtures" / "valid" / "evaluation-profile.json"
...
def core_worktree_status(repo: Path) -> dict[str, Any]:
    """Report status while excluding this additive extension sibling.

    The ASW core is the accepted baseline.  The parent Git worktree also
    contains this intentionally untracked extension package, so the manifest
    records both the core-only status and the full status rather than claiming
    a misleading globally clean tree.
    """
    ...
    extension_name = ROOT.name
    core_lines = [line for line in lines if extension_name not in line]
    return {
        "full_worktree_clean": not lines,
        "core_worktree_clean": not core_lines,
        "full_status": lines,
        "core_status": core_lines,
        "extension_status_excluded": [line for line in lines if line not in core_lines],
    }
