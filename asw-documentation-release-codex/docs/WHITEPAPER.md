...
Primary classes were job completion, file/artifact transition, and UI
transition. Secondary probes covered render/export-style output and
...
Layer A was a model-free systems benchmark with **zero model calls**. It
...
The release package provides a sanitized [run manifest](../evidence/RELEASE_RUN_MANIFEST.json),
[evidence summary](../evidence/RELEASE_EVIDENCE_SUMMARY.md), and
[claims matrix](CLAIMS_AND_EVIDENCE.md). The complete frozen run directory is
retained in the evaluation package under
...
If the origin is access-restricted, use the corresponding `v0.2.0` source
release archive. The evaluation package must be present at the path above; it
is a separate package directory in this workspace rather than a Python
...
Primary internal references are [RFC 0001](../../asw-spec-codex/RFC-0001.md),
the [runtime qualification](../../asw-spec-codex/docs/RUNTIME_QUALIFICATION_2026-08-02.md),
the [sanitized accepted evaluation report](../evidence/EVALUATION_RESULTS_2026-08-02.md),
and the [frozen aggregate](../evidence/aggregate-summary.json).
